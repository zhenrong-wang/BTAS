...
The introduced BitTree algorithm set aims to solve this problem with O(n) time complexity and lower/fixed space complexity. And the following contents would prove both the feasibility and superiority.

# 2. Algorithm Description

In order to record whether an element occurred or not previously, it is natural to design a seperate data structure, e.g. a Hash Table that transforms every raw element (in the integer case, is the whole integer or unsigned integer space with 2^32-1 possibilities) to an index table so that the inner loop described above would be avoided.
...
In order to resolve this issue, we need an Adjacent Index Hashmap to record the index. Obviously, this would introduce tramendous extra space cost because the index is in the unsigned int space (Let's omit 64bit because that's another story). Therefore, we need to make the Adjacent Index Hashmap dynamic.
...
- GROWING round: an perfectly sorted array with unsigned integers.
...
| BTS       |  0.001030  |  0.147594   |            |
| BTD       |  0.001934  |  0.187457   |            |
| BTSAIH    |  0.007222  |  0.817819   |            |
| BTDAIH    |  0.008861  |  0.784904   |            |
| BRUTE     |  4.608351  |             |            |
 *UNIQUE*       97533
