...
int32_t* import_1d_i32(const char *source_file, const char* type, uint32_t *num_elems_read, int *err_flag) {
    ...
    *num_elems_read = 0;
    ...
    if(file_type_flag == 0) {
        ...
        rewind(file_p);
        size_t num_elems_total = file_size / sizeof(int32_t);
        int32_t *array = (int32_t *)calloc(num_elems_total, sizeof(int32_t));
        if(array == NULL) {
            *err_flag = 1;
            fclose(file_p);
            return NULL;
        }
        size_t num_elems = fread(array, sizeof(int32_t), num_elems_total, file_p);
        *num_elems_read = (uint32_t)num_elems; /* TYPE RISK!!! */
        ...
        size_t i = 0, blocks = 1;
        char ch;
        int32_t *array_tmp = NULL;
        int32_t *array = (int32_t *)calloc(TXT_READ_BLOCK, sizeof(int32_t));
        if(array == NULL) {
            *err_flag = 1;
            fclose(file_p);
            return NULL;
        ...
        while(!feof(file_p)){
            ...
            if(i % TXT_READ_BLOCK == 0) {
                blocks++;
                array_tmp = (int32_t *)realloc(array, blocks * TXT_READ_BLOCK * sizeof(int32_t));
                if(array_tmp == NULL) {
                    *err_flag = 7;
                    free(array);
                    fclose(file_p);
                ...
                memset(array_tmp + (blocks - 1) * TXT_READ_BLOCK, 0, TXT_READ_BLOCK * sizeof(int32_t));
                array = array_tmp;
            }
        }
        fclose(file_p);
        ...
        *num_elems_read = i;
        array_tmp = (int32_t *)realloc(array, i * sizeof(int32_t));
        if(array_tmp == NULL) {
            return array;
        }
        else{
